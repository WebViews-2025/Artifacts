...
class WebViewChecker:
    def __init__(self, lock):
        ...
        self.base_webview_class = "Landroid/webkit/WebView;"
        self.base_results_file = ""
    ...
    def search_string_in_files(self, directory):
        ...
        command = f"find {directory} -type f -name '*.smali' -exec grep -l '{self.base_webview_class}' {{}} + > {tmp_file_path}"
        ...
        with open(tmp_file_path, "r") as tmp_file:
            for line in tmp_file:
                ...
                logging.info(f"Found WebView in: {file_path}")

        os.remove(tmp_file_path)

        logging.info(f"Found {len(matches)} WebViews in: {directory}")
        return matches
    ...
    def save_smali_code(self, app_id, file_paths, apk_content):
        if len(file_paths) == 0:
            logging.info("No WebViews found for: " + apk_content)
            return
        apk_content_dir = apk_content.split("/")[-1]
        renamed_filename = apk_content_dir.rsplit(".", 1)
        apk_content_dir = "_".join(renamed_filename)
    ...
    def complete_analysis(self, app_id, is_webview_present):
        with self.lock:
            ...
            results_df.loc[
                results_df["app_id"] == app_id, "webview_present"
            ] = is_webview_present
